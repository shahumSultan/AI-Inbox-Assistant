...
export default function ReplyPage() {
  ...
  return (
    <div className="flex flex-col px-8 py-10 max-w-3xl mx-auto w-full">
      ...
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-white text-3xl font-bold tracking-tight mb-1">Generate Reply</h1>
        {thread && (
          <p className="text-white/40 text-sm line-clamp-1">{thread.title}</p>
        )}
      </div>

      {/* Controls */}
      <div className="rounded-2xl border border-white/[0.08] p-6 mb-6" style={{ background: "rgba(255,255,255,0.025)" }}>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">

          {/* Goal */}
          <div>
            <label className="block text-white/50 text-xs font-medium mb-2">Goal</label>
          ...
          {/* Tone */}
          <div>
            <label className="block text-white/50 text-xs font-medium mb-2">Tone</label>
            <div className="flex flex-wrap gap-2">
              {TONES.map((t) => (
                <button
                  key={t.value}
        ...
        <div className="mb-5">
          <label className="block text-white/50 text-xs font-medium mb-2">
            Extra instruction <span className="text-white/20">(optional)</span>
          </label>
          <input
            type="text"
            value={extra}
            ...
            className="w-full px-3 py-2.5 rounded-xl border border-white/[0.08] bg-white/[0.04] text-white text-sm placeholder-white/20 outline-none focus:border-brand/40 transition-colors"
          />
        </div>
      ...
      {drafts.length > 0 && (
        <div className="flex flex-col gap-4">
          ...
          {/* Reason */}
          <p className="text-white/30 text-xs italic px-1">{drafts[active]?.reason}</p>
          ...
          <div className="rounded-2xl border border-white/[0.08] overflow-hidden" style={{ background: "rgba(255,255,255,0.025)" }}>
            <div className="flex items-center justify-between px-4 py-3 border-b border-white/[0.06]">
              <span className="text-white/30 text-xs font-medium uppercase tracking-wider">Reply draft</span>
              <div className="flex items-center gap-2">
                {hasEdit && (
                  <button
                    onClick={reset}
